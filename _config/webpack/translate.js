...
const taroCliSrc = '@tarojs/cli/src';

const Util = require(taroCliSrc + '/util');
const npmProcess = require(taroCliSrc + '/util/npm');
const CONFIG = require(taroCliSrc + '/config');
const { source: toAst } = require(taroCliSrc + '/util/ast_convert');
const PACKAGES = {
...
const entryFilePath = Util.resolveScriptPath(
    path.join(sourceDir, CONFIG.ENTRY)
);
const entryFileName = path.basename(entryFilePath);//入口文件是配置在taro/cli/src/config.ENTRY
...
const isUnderSubPackages = parentPath =>
    parentPath.isObjectProperty() &&
    /subPackages|subpackages/i.test(parentPath.node.key.name);
...
function processEntry(code, filePath) {
    ...
    const initPxTransformNode = toAst(
    ...
    ast = babel.transformFromAst(ast, '', {
        plugins: [
            [
                ...
                { ignore: ['@tarojs/taro', 'react', 'nervjs'] }
            ]
        ]
    }).ast;
    ...
    const programExitVisitor = {
        ClassMethod: {
            exit(astPath) {
                ...
                const isComponentWillUnmount =
                    key.name === 'componentWillUnmount';

                if (isRender) {
                    const pageRequires = pages
                        .map(v => {
              ...
              <${providorImportName} store={${storeName}}>
                ...
                if (tabBar && isComponentWillMount) {
                    const initTabBarApisCallNode = toAst(
                        `Taro.initTabBarApis(this, Taro)`
                    );
                    astPath
                        .get('body')
                        .pushContainer('body', initTabBarApisCallNode);
                }

                if (hasComponentDidShow && isComponentDidMount) {
                    const componentDidShowCallNode = toAst(
                        `this.componentDidShow()`
                    );
                    astPath
                        .get('body')
                        .pushContainer('body', componentDidShowCallNode);
                }

                if (hasComponentDidHide && isComponentWillUnmount) {
                    const componentDidHideCallNode = toAst(
                        `this.componentDidHide()`
                    );
                    astPath
                        .get('body')
                        .unshiftContainer('body', componentDidHideCallNode);
                }
            }
        },
        ClassProperty: {
            exit(astPath) {
                ...
                if (key.name !== 'state' || !t.isObjectExpression(value))
                    return;
                astPath.node.value.properties.push(
                    t.objectProperty(t.identifier(tabBarConfigName), tabBar)
                );
            }
        },
        ClassBody: {
        ...
        ObjectProperty(astPath) {
            ...
            if (keyName === 'pages' && t.isArrayExpression(value)) {
                ...
                value.elements.forEach(v => {
                    const pagePath = `${root}/${v.value}`.replace(
                        /\/{2,}/g,
                        '/'
                    );
                    pages.push(pagePath);
                ...
                value.properties.forEach(node => {
                    if (node.keyName === 'position')
                        tabbarPos = node.value.value;
                });
            } else if (
                (keyName === 'iconPath' || keyName === 'selectedIconPath') &&
    ...
    traverse(ast, {
            ...
            enter(astPath) {
                ...
                if (value === PACKAGES['@tarojs/taro']) {
                    ...
                    if (specifier) {
                        ...
                        node.specifiers.unshift(
                            t.importDefaultSpecifier(
                                t.identifier(nervJsImportDefaultName)
                            )
                        );
                    }
                    ...
                    specifiers.forEach((item, index) => {
                        if (
                            item.imported &&
                            taroApis.indexOf(item.imported.name) >= 0
                        ) {
                    ...
                    if (taroApisSpecifiers.length) {
                        astPath.insertBefore(
                            t.importDeclaration(
                                taroApisSpecifiers,
                                t.stringLiteral(PACKAGES['@tarojs/taro-h5'])
                            )
                        );
                    }
                    ...
                    const specifier = specifiers.find(item => {
                        return (
                            ...
                            item.imported.name === providerComponentName
                        );
                    });
                    if (specifier) {
                        ...
                        specifiers.push(
                            t.importSpecifier(
                                t.identifier(providerComponentName),
                                t.identifier(providerComponentName)
                            )
                        );
                    }
        ...
        Program: {
            exit(astPath) {
                const importNervjsNode = t.importDefaultSpecifier(
                    t.identifier(nervJsImportDefaultName)
                );
                const importRouterNode = toAst(
                    `import { ${routerImportName} } from '${
                        PACKAGES['@tarojs/router']
                    }'`
                );
                const importTaroH5Node = toAst(
                    `import ${taroImportDefaultName} from '${
                        PACKAGES['@tarojs/taro-h5']
                    }'`
                );
                const renderCallNode = toAst(renderCallCode);
                const importComponentNode = toAst(
                    `import { View, ${tabBarComponentName}, ${tabBarContainerComponentName}, ${tabBarPanelComponentName}} from '${
                        PACKAGES['@tarojs/components']
                    }'`
                );
                const lastImportIndex = _.findLastIndex(
                    astPath.node.body,
                    t.isImportDeclaration
                );
                const lastImportNode = astPath.get(
                    `body.${lastImportIndex > -1 ? lastImportIndex : 0}`
                );
                const extraNodes = [
                    importTaroH5Node,
                    importRouterNode,
...
function processFiles(filePath) {
    ...
    const distDirname = dirname.replace(sourceDir, tempDir);//替换输出目录为.temp目录
...
function buildTemp() {
    ...
    return new Promise((resolve, reject) => {
        const relativePath = path.relative(appPath, sourceDir);
        Util.printLog(
            Util.pocessTypeEnum.CREATE,
            `遍历[/${relativePath}]文件夹，处理转换js脚本，所有文件最终输出到[/${tempDir}]文件夹`
        );
        //递归目录查找文件
        klaw(sourceDir)
            .on('data', file => {
...
async function buildDist(buildConfig) {
    ...
    const entryFile =
        path.basename(entryFileName, path.extname(entryFileName)) + '.js';
